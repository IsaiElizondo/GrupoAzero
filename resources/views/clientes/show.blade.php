                        ...
                        @if($cliente->direcciones->count() > 0)
                            @foreach($cliente->direcciones as $dir)
                                <div class="card p-3 mb-2">
                                    <h4>{{ $dir->nombre_direccion }} </h4>
                                    <strong>{{ $dir->direccion }}</strong>
                                    <p class="mb-1">{{ $dir->ciudad }}, {{ $dir->estado }} - CP: {{ $dir->codigo_postal }} - Celular: {{$dir->celular}}</p>
                                    @if($dir->url_mapa)
                                        <p class="mb-1">
                                    ...
                                    @if($dir->instrucciones)
                                        <p class="text-muted">Instrucciones: {{ $dir->instrucciones }}</p>
                                    @endif
                                </div>
                            @endforeach
                        @else
