            ...
            <a href="{{ url('pedidos2/pedido/'.$item->id) }}">  
                ...
                {{-- =================================== --}}
                    ...
                    @php
                        ...
                        $OfsVisibles = $ordersf->filter(function($of){
                            if(auth()->user()->department_id == 5){
                                return in_array($of->status_id, [1,3]) && $of->officeCreated() == auth()->user()->office;
                            }
                            return true;
                        });
                    @endphp
                ...
                <div rel="icons" >
                    ...
                    {{-- ============================== --}}
                        ...
                        @if($item->recibido_embarques_at && in_array($item->status_id, [2, 5]))
                            @if (in_array(auth()->user()->role->name, ["Administrador","Empleado"]) && auth()->user()->department_id != 5)
                    ...
                    {{-- ================================== --}}
                    {{-- ===== FIN CONTADOR EMBARQUES ===== --}}
                    {{-- ================================== --}}

                    
                    {{-- ================================ --}}
                    {{-- ===== CONTADOR FABRICACION ===== --}}
                    {{-- ================================ --}}

                        @if(auth()->user()->department_id == 5 && $OfsVisibles->count() > 0)                                    
                            @if($OfsVisibles->count() == 1)
                                @php
                                    $of = $OfsVisibles->first();
                                    $InicioFab = \Carbon\Carbon::parse($of->created_at);
                                    $HorasFab = $InicioFab->diffInHours(now());
                                @endphp
                                ...
                                <div class="datito mt-1">
                                    ...
                                    <span class="dias-contador" style="color: #000080;">
                                        <center>
                                            ...
                                            @else
                                                {{ number_format($InicioFab->floatDiffInRealDays(now()), 2) }} días
                                            @endif
                                        </center>
                                    </span>
                                </div>
                            ...
                            @if($OfsVisibles->count() > 1)
                                @php
                                    $OfMasAntigua = $OfsVisibles->sortBy('created_at')->first();
                                    $InicioFab = \Carbon\Carbon::parse($OfMasAntigua->created_at);
                                    $HorasFab = $InicioFab->diffInHours(now());
                                @endphp
                            ...
                            @if($OfsVisibles->count() > 1)
                                <div class="modal fade" id="ModalFabricacion{{$item->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                                    <div class="modal-dialog modal-md" role="document">
                                        <div class="modal-content p-3">

                                            <div class="modal-header">
                                                <h5 class="modal-title"> Seguimiento de Fabricación </h5>
                                                ...
                                                <table class="table table-bordered table-sm mb-0">
                                                    ...
                                                    <tbody>
                                                        @foreach($OfsVisibles as $of)
                                                            ...
                                                            <tr>
                                                                <td>{{ $of->number }}</td>
                                                                <td>{{ $InicioFab->format('d-m-Y H:i') }}</td>
                                                                <td>
                                                                    @if($HorasFab < 24)
                                                                        {{ number_format($HorasFab, 2) }} hrs
                                                                    @else
